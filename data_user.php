...
<div class="preloader">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<?php 
			...
			$perintah = "SELECT * FROM `data_user`";
							...
							<select name="komod" class="form-control">
								...
								<?php 
									while ($komod=mysqli_fetch_array($p1)) {
										echo "<option value='$komod[id_user]'>$komod[nama_user]</option>";
									}
								 ?>
							</select>
							</select>
		...
		if (isset($_POST['tombol'])) {
			$id = $_POST['komod'];
			$perintah2 = "SELECT u.nama_user, SUM(p.total_bayar) as Total_Bayar, 
			COUNT(p.id_user) as Jumlah_Pesanan FROM data_user u, data_permohonan p 
			WHERE p.id_user = '$id' 
			AND u.id_user = '$id'";
		 ...
		 ?>
						<table data-toggle="table"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="nomor">
						    ...
						    <tbody>
						    	<?php 
						    		$No = 1;
						    		while ($permohonan=mysqli_fetch_row($exucute)) {
						    	?>
						    	<tr>
						    		<td><?php echo $No++; ?></td>
						    		<td><?php echo "$permohonan[0]"; ?></td>
						    		<td><?php echo "$permohonan[2]"; ?></td>
						    		<td><?php echo "Rp. $permohonan[1]"; ?></td>
						    	</tr>
						    	<?php 
						    		}
						    	 ?>
