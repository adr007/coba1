...
<div class="preloader">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<?php 
			...
			//$perintah = "SELECT DISTINC FROM `data_permohonan`";
		...
		if (isset($_POST['tombol'])) {
			$id = $_POST['komod'];
			$perintah2 = "SELECT u.nama_user, k.nama_umum, e.nama_ekspedisi, p.status 
				FROM data_user u, data_komoditi k, data_ekspedisi e, data_permohonan p 
				WHERE p.status = '$id' 
				AND u.id_user = p.id_user 
				AND e.id_ekspedisi = p.id_ekspedisi 
				AND k.id_komoditi = p.id_komoditi";
			$exucute = mysqli_query($con, $perintah2);
			$perintah3 = "SELECT COUNT(u.nama_user)as jumlah 
				FROM data_user u, data_komoditi k, data_ekspedisi e, data_permohonan p 
				WHERE p.status = '$id' 
				AND u.id_user = p.id_user 
				AND e.id_ekspedisi = p.id_ekspedisi 
						...
						<table data-toggle="table"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="nomor">
						    ...
						    <tbody>
						    	...
						    	<tr>
						    		...
						    		<td><?php echo "$permohonan[nama_ekspedisi]"; ?></td>
